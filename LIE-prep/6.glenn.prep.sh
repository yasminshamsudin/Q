...
#---------- RUNTIME with 2fs time steps becomes DOUBLE the below collection time! ------------#
...
# walltime=19:00:00 runtime=400000 # 4ns collection (4000 steps/min in 20A sphere)
 walltime=36:00:00 runtime=500000 # 5ns collection (4000 steps/min in 20A sphere)
# walltime=60:00:00 runtime=1000000 # 10ns collection (4000 steps/min in 20A sphere)
# walltime=72:00:00 runtime=12500000 # 12.5ns collection (4000 steps/min in 20A sphere)
# walltime=84:00:00 runtime=15000000 # 15ns collection (4000 steps/min in 20A sphere)
...
#---------------END OF VARIABLES SECTION- NO MORE EDITING! -----#
 ...
 sed -i "s/500000/$runtime/g" $ligname/Protein/md*.inp
...
# Create job-files
 ...
 for i in {1..5}
 do
        echo -e "$mpi $Qdyn5p eq$i.inp >"' ${job}/'"eq$i.log" >> $ligname/Protein/$date.prot.$ligname.job
 done
